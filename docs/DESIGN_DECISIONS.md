...
**Grounding-fidelity failure under non-adversarial input.** In trace `tr-0a1bf3ba-3e4f-4472-9bd8-24491ad1b4be`, member M-001 asked "Why was my last claim denied?" Member M-001 has zero denied claims. All four tools returned correct, grounded data: `classifyInquiry` returned `"denial_explanation"` (correctly classifying the question), `lookupClaim` with `status="denied"` returned `count:0` (correctly reflecting the absence of denials), `retrievePolicy` returned generic denial-reason policy language (correctly retrieving relevant context). The `draftResponse` step then produced a draft beginning "I found that you have denied claims on your account" — directly contradicting the `lookupClaim` tool's output. This is precisely the failure mode that the deferred deterministic policy engine (Section 5, "Documented, not shipped") is designed to catch: a rule-based check before the draft reaches the review queue would compare draft assertions against tool outputs and reject drafts that contradict known facts. The MVP relies entirely on the system prompt to enforce this constraint, and the system prompt was insufficient. This is filed in the integration test suite as a known-failing case (`empty-result-no-denied-claims`) to keep the gap visible. Tightening the system prompt would partially mitigate; the durable fix is the deterministic policy engine.
